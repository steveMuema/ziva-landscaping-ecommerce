...
const LABELS: Record<string, string> = {
  ...
  [SETTING_KEYS.DELIVERY_FEE_KSH]: "Delivery fee (KSH)",
...
export default function SettingsForm() {
      ...
      .then(async (r) => {
        ...
        if (data.settings) {
          ...
          for (const key of Object.values(SETTING_KEYS)) {
            initial[key] = data.settings[key] ?? (key === SETTING_KEYS.MPESA_ENV ? "sandbox" : key === SETTING_KEYS.DELIVERY_FEE_KSH ? "400" : "");
          }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const keysToAlwaysSend: string[] = [SETTING_KEYS.SITE_LOGO_URL, SETTING_KEYS.SITE_PARALLAX_IMAGE_URL];
        ...
        <nav className="flex gap-0.5" aria-label="Settings sections">
          {tabs.map((tab) => (
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-t-lg border-b-2 px-4 py-3 text-sm font-medium transition-colors ${activeTab === tab.id
        ...
        {activeTab === "api" && (
          <div className="space-y-6 p-5">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/50 shadow-sm">
              ...
              <div className="p-5 space-y-4">
                <div>
                  <label htmlFor={SETTING_KEYS.GEMINI_API_KEY} className="block text-sm font-medium text-slate-700">
                    {LABELS[SETTING_KEYS.GEMINI_API_KEY]}
                  </label>
                  <div className="mt-1 flex items-center gap-2">
                    <input
                      id={SETTING_KEYS.GEMINI_API_KEY}
                      type="password"
                      autoComplete="off"
                      placeholder={settings[SETTING_KEYS.GEMINI_API_KEY] ? "•••••••• (leave blank to keep)" : "Not set"}
                      ...
                      value={form[SETTING_KEYS.GEMINI_API_KEY] ?? ""}
                      onChange={(e) => setForm((p) => ({ ...p, [SETTING_KEYS.GEMINI_API_KEY]: e.target.value }))}
                    />
                    {settings[SETTING_KEYS.GEMINI_API_KEY] && (
          ...
          <div className="space-y-6 p-5">
            <div className="rounded-xl border border-slate-200 bg-slate-50/50 shadow-sm">
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  <label htmlFor={SETTING_KEYS.DELIVERY_FEE_KSH} className="block text-sm font-medium text-slate-700">{LABELS[SETTING_KEYS.DELIVERY_FEE_KSH]}</label>
                  <input
                    id={SETTING_KEYS.DELIVERY_FEE_KSH}
                    type="number"
                    min="0"
                    step="1"
                    ...
                    value={form[SETTING_KEYS.DELIVERY_FEE_KSH] ?? "400"}
                    onChange={(e) => setForm((p) => ({ ...p, [SETTING_KEYS.DELIVERY_FEE_KSH]: e.target.value }))}
                    placeholder="400"
                  />
                  <p className="mt-1 text-xs text-slate-500">Shown at checkout and applied to each order. Default KSH 400.</p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6 p-5">
            <div className="rounded-xl border border-slate-200 bg-slate-50/50 shadow-sm">
              ...
              <div className="p-5 space-y-4">
                {profileMessage && (
                  <div
                    className={`rounded-lg border p-4 ${profileMessage.type === "success"
                      ? "border-emerald-200 bg-emerald-50 text-emerald-800"
                      : "border-red-200 bg-red-50 text-red-800"
                      }`}
                  >
