...
/** Agriculture module: categories clearly separated by isAgriculture flag */
export async function getAgricultureCategories() {
  try {
    const categories = await prisma.category.findMany({
      where: { isAgriculture: true },
      include: { subCategories: { include: { products: true } } },
      orderBy: { id: "asc" },
    });
    return categories;
...
/** Products for slideshow: featured tag first, then recent, limited by count */
export async function getProductsForSlideshow(limit = 12) {
  try {
    ...
    const recent = await prisma.product.findMany({
      where: featured.length ? { id: { notIn: featured.map((p) => p.id) } } : undefined,
      take: remaining,
